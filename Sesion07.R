...
dbListFields(database, 'CountryLanguage')

# Consulta tipo MySQL

df <- dbGetQuery(database, "select * from CountryLanguage;")
class(df)
head(df)
...
df_spanish %>% ggplot(aes( x = CountryCode, y=Percentage, fill = IsOfficial )) + 
  geom_bin2d() +
  coord_flip()
